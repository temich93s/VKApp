...
/// - parameterEncodingFailed:     Returned when a parameter encoding object throws an error during the encoding
/// process.
...
extension Error {
    var underlyingAdaptError: Error? { (self as? AdaptError)?.error }
}
...
public extension AFError {
    ...
    /// Returns whether the AFError is a multipart encoding error. When `true`, the `url` and `underlyingError`
    /// properties
