...
open class Request {
    ...
    /// The underlying task.
    open var task: URLSessionTask? { delegate.task }

    /// The session belonging to the underlying task.
    public let session: URLSession

    /// The request sent or to be sent to the server.
    open var request: URLRequest? { task?.originalRequest }

    /// The response received from the server, if any.
    open var response: HTTPURLResponse? { task?.response as? HTTPURLResponse }
    ...
    func cURLRepresentation() -> String {
        ...
        if let credentialStorage = session.configuration.urlCredentialStorage {
            ...
            if let credentials = credentialStorage.credentials(for: protectionSpace)?.values {
                ...
                if let credential = delegate.credential, let user = credential.user,
...
open class DataRequest: Request {
    ...
    open var progress: Progress { dataDelegate.progress }

    var dataDelegate: DataTaskDelegate { delegate as! DataTaskDelegate }
...
open class DownloadRequest: Request {
    ...
    open var resumeData: Data? { downloadDelegate.resumeData }

    /// The progress of downloading the response data from the server for the request.
    open var progress: Progress { downloadDelegate.progress }

    var downloadDelegate: DownloadTaskDelegate { delegate as! DownloadTaskDelegate }
...
open class UploadRequest: DataRequest {
    ...
    open var uploadProgress: Progress { uploadDelegate.uploadProgress }

    var uploadDelegate: UploadTaskDelegate { delegate as! UploadTaskDelegate }
