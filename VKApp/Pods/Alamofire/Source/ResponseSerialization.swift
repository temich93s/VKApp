...
public extension DownloadRequest {
    ...
    static func dataResponseSerializer() -> DownloadResponseSerializer<Data> {
        DownloadResponseSerializer { _, response, fileURL, error in
            guard error == nil else { return .failure(error!) }
...
public extension Request {
        ...
        -> Result<String>
    {
        ...
        if let string = String(data: validData, encoding: actualEncoding) {
            ...
            return .failure(
                AFError
                    .responseSerializationFailed(reason: .stringSerializationFailed(encoding: actualEncoding))
            )
        }
    }
}
...
public extension DownloadRequest {
        ...
        -> DownloadResponseSerializer<Any>
    {
        DownloadResponseSerializer { _, response, fileURL, error in
            ...
            do {
                let data = try Data(contentsOf: fileURL)
                return Request.serializeResponsePropertyList(
                    options: options,
                    response: response,
                    data: data,
                    error: error
