...
open class SessionDelegate: NSObject {
    ...
    open var sessionDidReceiveChallenge: (
        (URLSession, URLAuthenticationChallenge)
            -> (URLSession.AuthChallengeDisposition, URLCredential?)
    )?

    /// Overrides all behavior for URLSessionDelegate method `urlSession(_:didReceive:completionHandler:)` and requires
    /// the caller to call the `completionHandler`.
    open var sessionDidReceiveChallengeWithCompletion: ((
        URLSession,
    ...
    open var taskWillPerformHTTPRedirectionWithCompletion: ((
        URLSession,
        URLSessionTask,
        HTTPURLResponse,
        URLRequest,
    ...
    open var taskDidReceiveChallenge: (
    ...
    /// requires the caller to call the `completionHandler`.
    open var taskDidReceiveChallengeWithCompletion: ((
        URLSession,
        URLSessionTask,
        URLAuthenticationChallenge,
        @escaping (URLSession.AuthChallengeDisposition, URLCredential?) -> Void
    ...
    /// requires the caller to call the `completionHandler`.
    open var taskNeedNewBodyStreamWithCompletion: (
        (URLSession, URLSessionTask, @escaping (InputStream?) -> Void)
            -> Void
    )?
    ...
    open var dataTaskDidReceiveResponse: (
        (URLSession, URLSessionDataTask, URLResponse) -> URLSession
            .ResponseDisposition
    )?
    ...
    open var dataTaskDidReceiveResponseWithCompletion: ((
        URLSession,
        URLSessionDataTask,
        URLResponse,
    ...
    /// `urlSession(_:dataTask:willCacheResponse:completionHandler:)` and
    /// requires caller to call the `completionHandler`.
    open var dataTaskWillCacheResponseWithCompletion: ((
        URLSession,
        URLSessionDataTask,
        CachedURLResponse,
    ...
    open var streamTaskReadClosed: ((URLSession, URLSessionStreamTask) -> Void)? {
        get {
            _streamTaskReadClosed as? (URLSession, URLSessionStreamTask) -> Void
        }
        set {
    ...
    open var streamTaskWriteClosed: ((URLSession, URLSessionStreamTask) -> Void)? {
        get {
            _streamTaskWriteClosed as? (URLSession, URLSessionStreamTask) -> Void
        }
        set {
    ...
    open var streamTaskBetterRouteDiscovered: ((URLSession, URLSessionStreamTask) -> Void)? {
        get {
            _streamTaskBetterRouteDiscovered as? (URLSession, URLSessionStreamTask) -> Void
        }
        set {
    ...
    /// Overrides default behavior for URLSessionStreamDelegate method
    /// `urlSession(_:streamTask:didBecome:outputStream:)`.
    @available(iOS 9.0, macOS 10.11, tvOS 9.0, *)
    open var streamTaskDidBecomeInputAndOutputStreams: (
        (URLSession, URLSessionStreamTask, InputStream, OutputStream)
            -> Void
    )? {
        get {
            _streamTaskDidBecomeInputStream as? (URLSession, URLSessionStreamTask, InputStream, OutputStream) -> Void
        }
        set {
    ...
    ///
    /// - returns: `true` if the receiver implements or inherits a method that can respond to selector, otherwise
    /// `false`.
...
extension SessionDelegate: URLSessionTaskDelegate {
    ...
    /// - parameter bytesSent:                The number of bytes sent since the last time this delegate method was
    /// called.
