...
open class SessionManager {
    ...
    /// If `startRequestsImmediately` is `true`, the request will have `resume()` called before being returned.
    ///
    /// On the latest release of all the Apple platforms (iOS 10, macOS 10.12, tvOS 10, watchOS 3), `resumeData` is
    /// broken
    /// on background URL session configurations. There's an underlying bug in the `resumeData` generation logic where
    /// the
    ...
    open func download(
        ...
        -> DownloadRequest
    {
        download(.resumeData(resumeData), to: destination)
    }
        ...
        DispatchQueue.global(qos: .utility).async {
            ...
            do {
                ...
                if formData.contentLength < encodingMemoryThreshold, !isBackgroundSession {
                    ...
                    let directoryURL = tempDirectoryURL
    ...
    open func stream(withHostName hostName: String, port: Int) -> StreamRequest {
        stream(.stream(hostName: hostName, port: port))
    }
